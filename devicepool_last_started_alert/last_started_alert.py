...
class LastStarted:
    ...
    def trigger_event(self):
        ...
        summary = "Jobs are present in queues, but no 'started' lines in devicepools logs for %s minutes!" % MINUTES_OF_LOGS_TO_INSPECT
    ...
    def get_journalctl_output(self):
        cmd = "sudo journalctl -u bitbar --since '%s minutes ago'" % MINUTES_OF_LOGS_TO_INSPECT
...
if __name__ == "__main__":
    ...
    currently_alerting = ls.currently_alerting()
    jobs_in_queues = ls.jobs_in_queues()
    started_lines_present = ls.started_lines_present()
    completed_lines_present = ls.completed_lines_present()
    enough_journalctl_lines = ls.enough_journalctl_lines()
    ...
    if args.verbose:
        ...
        print("Lines of journalctl output: %s" % ls.journalctl_lines_of_output)
    ...
    if ls.alerting_enabled:
        ...
        # TESTING
        # dk = ls.create_dedup_key()
        # ls.set_dedup_key(dk)
        # print(ls.get_dedup_key())
        # ls.set_currently_alerting()
        #
        # ls.trigger_event()
        # ls.resolve_incident()
